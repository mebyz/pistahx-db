...
AutoSequelize.prototype.run = function(options, callback) {
  ...
  this.queryInterface.showAllTables().then(function (__tables) {
    ...
    async.each(tables.sort(), mapForeignKeys4, mapTables4)
  });
  ...
  function generateText(err) {
    ...
    async.each(tableNames, function(table, _callback){
      ...
      text[table] += spaces + spaces  + "tableName: '" + table + "', timestamps: false,\n";
...
function mapTables4(err) {
  ...
  function generateText4(err) {
    ...
    var tableNames = _.keys(_tables).sort();
    text2[0] += "import promhx.Promise.Promise;\n\n";
    text2[1] +="@:publicFields\n";
    text2[1] += "class DbRepos {\n\n";        
    text2[2] += "\tfunction new(db : Sequelize) {\n\n";
    ...
    async.each(tableNames, function(table, _callback){
      ...
      text2[2] += "\t\tdb" + table + " = db.import_(\"models/" + table + ".js\");\n";


      text2[0] += "typedef DB" + table + " = {\n";
      text2[0] += "\tfindAll : FindAllOptions -> Promise<Array<DB__" + table + ">>,\n";
      text2[0] += "\tfind : FindOptions -> Promise<DB__" + table + ">,\n";
      text2[0] += "\tbuild : Dynamic,\n";
      text2[0] += "\tsave : Dynamic,\n";
      text2[0] += "\tthen : Dynamic,\n";
      text2[0] += "\thasOne : Dynamic,\n";
      text2[0] += "\thasMany : Dynamic,\n";
      text2[0] += "\tbelongsToMany : Dynamic,\n";
      text2[0] += "\tbelongsTo : Dynamic\n";
      text2[0] += "}\n\n";

      text2[1] += "\tvar db" + table + " : DB" + table + ";\n";
      ...
      for (var x = 0; x < options.indentation; ++x) {
        spaces += (options.spaces === true ? ' ' : "\t");
            ...
            //text2[0] += spaces + spaces +"module.exports = function(sequelize) {\n";
            //text2[0] += spaces + spaces + spaces + "var m = sequelize.import('./"+table+".js');\n";
      ...
      _.each(fields, function(field, i){
        ...
        _.each(fieldAttr, function(attr, x){
          ...
          if (attr === "foreignKey") {
            ...
            if (!donefk){
              text2[2] += "\t\tdb"+table+".hasOne(db"+_tables[table][field][attr].target_table+", {foreignKey: '"+_tables[table][field][attr].target_column+"', as: '"+_tables[table][field][attr].target_table+"' });\n";
              donefk=!donefk;
            }
            //text2[0] += spaces + spaces + spaces + "var "+_tables[table][field][attr].target_table+" = sequelize.import('./"+_tables[table][field][attr].target_table+".js');\n";
            ...
            //text2[0] += spaces + spaces + spaces + "return m;\n";
            //text2[0] += spaces + spaces + "};\n";
...
AutoSequelize.prototype.write4 = function(attributes, callback) {
  ...
  fs.writeFile(self.options.directory + '/DbRepos.hx',text2[0].concat(text2[1]).concat(text2[2].concat(text2[3])));
